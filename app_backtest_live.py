...
with st.sidebar:
    ...
    universe_type = st.selectbox("标的类型", ["stock", "industry"], format_func=lambda x: "股票" if x == "stock" else "行业指数")
    ...
    if universe_type == "stock":
        ...
        industry_codes_text = st.text_input("行业指数代码过滤", value="", placeholder="例如 881121,881273")
        industry_index_codes = parse_code_list(industry_codes_text) or None

    stock_max_number = st.number_input("最大标的数（-1 为全部）", value=-1, step=1)
    max_workers = st.number_input("加载进程数", min_value=1, max_value=16, value=1, step=1)
...
with tab_strategy:
    ...
    selected_strategies = st.multiselect(
        ...
        default=[strategy_options[0]] if strategy_options else [],
    ...
    st.subheader("组合信号")
    signal_combination = st.selectbox("组合方式", ["weighted", "voting", "unanimous"], index=0)
    signal_threshold = st.number_input("信号阈值", value=0.5, min_value=0.0, max_value=5.0, step=0.05)
    signal_weights = []
    if selected_strategies:
    ...
    enable_stop_profit = st.checkbox("启用止盈", value=True)
...
with tab_symbol:
    ...
    if not signaled_data:
        ...
        with st.expander("信号数据预览", expanded=False):
            preview_cols = [col for col in ["open", "high", "low", "close", "volume", "signal"] if col in signaled_data[symbol].columns]
